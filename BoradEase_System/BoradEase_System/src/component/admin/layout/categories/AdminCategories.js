...
const AdminCategories = () => {
    ...
    const deleteCategory = (e, id) => {
        ...
        axios.delete(BASE_URL+'/api/categories/' + id,{
            ...
            .then(res => {
                setCategory(prevProducts => prevProducts.filter(product => product.id !== id));
                Swal.fire({
                    position: 'top-end',
                    icon: 'success',
                                    ...
                                    : getCategory.length > 0
                                        ? getCategory.map((category, index) => (
                                            <tr key={category.id} className='text-center'>
                                                ...
                                                <td><img src={`${BASE_URL}/categories/${category.image}`} className='img-fluid' alt={`${category.name} Logo`} width="50px" /></td>
                                                <td className='text-secondary'>{category.name}</td>
                                            
                                                            <td className='text-secondary text-lowercase'>{category.description}</td>
                                                            <td className='text-center text-secondary'>
                                                                <Link to={`/auth/view-category/${category.id}`} className='btn btn-warning btn-table'>
                                                                    <i className='bx bx-show'></i> View
                                                                </Link>
                                                            </td>
                                                            <td className='text-center'>
                                                                <Link to={`/auth/update-category/${category.id}`} className='btn btn-success btn-table'>
                                                                    <i className='bx bxs-edit'></i> Update
                                                                </Link>
                                                            </td>
                                                            <td className='text-center'>
                                                                <button onClick={(e) => deleteCategory(e, category.id)} className='btn btn-danger btn-table'>
